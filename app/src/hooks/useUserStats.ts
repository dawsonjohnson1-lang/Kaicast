...
// Mirrors desktop/hooks/useUserStats.ts so both surfaces read the same
// shape from the same path — neither client computes stats at render.
...
export function useUserStats(uid: string | null | undefined): State {
  const [state, setState] = useState<State>({ stats: null, loading: !!uid });
    ...
    const unsub = onSnapshot(
      ...
      (snap) => {
        if (!snap.exists()) {
          ...
          // haven't logged a dive. Surface zeros so the UI doesn't have
