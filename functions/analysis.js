...
/**
 * Compute rolling rainfall totals ending at `nowMs` for windows of
 * 3h, 6h, 12h, 24h, 48h, and 72h.
 *
 * @param {object}   opts
 * @param {Array}    opts.hourlyItems  - Normalized hourly items with { tsMs, rainLast1hMM }
 * @param {number}  [opts.nowMs]       - Reference timestamp (ms). Defaults to Date.now().
...
function computeRainTotals({ hourlyItems = [], nowMs } = {}) {
  ...
  for (const item of hourlyItems) {
    ...
    if (ts > now) continue; // ignore future items
    const ageMs = now - ts;
    if (ageMs > maxWindowMs) continue; // ignore items older than 72h

    const mm = Number.isFinite(item.rainLast1hMM) ? item.rainLast1hMM : 0;

    if (ageMs <= 3 * H) rain3h += mm;
    if (ageMs <= 6 * H) rain6h += mm;
    if (ageMs <= 12 * H) rain12h += mm;
    if (ageMs <= 24 * H) rain24h += mm;
    if (ageMs <= 48 * H) rain48h += mm;
    rain72h += mm;
  }

  return {
    rain3hMM:  round1(rain3h),
    rain6hMM:  round1(rain6h),
    rain12hMM: round1(rain12h),
    rain24hMM: round1(rain24h),
    rain48hMM: round1(rain48h),
    rain72hMM: round1(rain72h),
  };
}
