// vesselSummary — a 1–2 sentence, plain-language read on how a specific
// hull will handle a specific spot on a specific day. Skipper voice, not
// a field dump. The phrasing is driven by the dominant condition + the
// vessel's behavior profile, so a cat and a RIB on the same forecast get
// genuinely different reads.
//
// MIRROR: app/src/charter/vesselSummary.ts. Keep in sync.
...
/** Build the per-vessel summary. Returns '' only when there's no data. */
export function summaryFor(
  ...
  // ── Sentence 1: the ride for this hull ──
  let lead: string;
  switch (risk) {
    case 'Low':
      lead = `${cap(f.plural)} will have a comfortable ride on ${seaPhrase(c)}`;
      break;
    case 'Moderate':
      lead = `${cap(f.plural)} will be fine on ${seaPhrase(c)}, with some motion`;
      break;
    case 'High':
      lead = `${cap(f.plural)} are in for a sporty ride in ${seaPhrase(c)}`;
      break;
    case 'Extreme':
      lead = `It's a beating out there for ${f.plural} — ${seaPhrase(c)} is past what they're happy in`;
      break;
    default:
      lead = `${cap(f.plural)} should be okay on ${seaPhrase(c)}`;
  }

  // Forgiving hulls get a reassurance when conditions are short-period but
  // within their comfort band; punishing hulls get the warning.
  ...
  // ── Sentence 2: timing / advice / hull-specific concern ──
  const advice: string[] = [];
  ...
  if (c.boxJelly?.open) {
    advice.push(`mind the south-shore box-jelly window`);
  }

  if (advice.length === 0) return `${lead}.`;
  const tail = advice.join(', ');
  return `${lead}. ${cap(tail)}.`;
}
